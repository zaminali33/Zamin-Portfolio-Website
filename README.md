# Zamin Ali — Portfolio Website

Personal portfolio website of **Zamin Ali**, a passionate Web Developer & AI Enthusiast and BSCS student from Islamabad, Pakistan.

## 🌐 Live Website
[zaminali33.github.io/portfolio](https://zaminali33.github.io/portfolio)

## 👤 About Me
I'm **Zamin Ali**, a BSCS student at FUAST Islamabad. I build modern, responsive websites using HTML, CSS, JavaScript, Python, and AI tools. I deliver fast and affordable websites for businesses using the power of AI.

## ✨ Features
- 🌙 Dark / Light theme toggle
- 🤖 AI Chatbot — ask anything about Zamin
- 📱 Fully responsive — works on all devices
- ⚡ Fast loading — clean code
- 🎨 Modern UI with smooth animations

## 🛠️ Built With
- HTML5
- CSS3
- JavaScript
- AI Tools (Claude, ChatGPT, Framer AI)
- GitHub Pages (Hosting)

## 🎓 Education
- 🏫 Scholar School
- 📚 FSc — Degree College Skardu
- 🎓 BSCS — FUAST Islamabad (Currently)

## 🗣️ Languages
Balti · Urdu · English

## 📂 My Projects
| Project | Status | Link |
|---------|--------|------|
| Delicious Flavor | ✅ Live | [View](https://zaminali33.github.io/delicious-flavor-website/) |
| Spice Symphony | ✅ Live | [View](https://zaminali33.github.io/spice-symphony-website-/) |

## 📬 Contact
- 📧 Email: zaminn.alii.33@gmail.com
- 💬 WhatsApp: +92 355 4467376
- 🐙 GitHub: [github.com/zaminali33](https://github.com/zaminali33)
